...
class MINDAllIteratorNotDictTitle(BaseIterator):
    ...
    def init_news(self, news_file):
        ...
        print("title shape", len(news_title), news_title[0])

        for news_index in range(len(news_title)):
            
            title = news_title[news_index]
            # print("lay ra title la ", title)
            ab = news_ab[news_index]
            vert = news_vert[news_index]
            subvert = news_subvert[news_index]
            # for word_index in range(min(self.title_size, len(title))):  # xét tất cả các từ trong title
                # if title[word_index] in self.word_dict:     # nếu từ đó có trong từ điển 
            ...
            # if len(sentence)>0:
            #     print("tien hanh ley embbeding ")
            #     encoded_input = tokenizer(sentence, return_tensors='pt')
            #     output = model_bert(**encoded_input) 
            #     self.news_title_index[news_index] = output
