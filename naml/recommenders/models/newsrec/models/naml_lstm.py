...
class NAMLModel(BaseModel):
    ...
    def _build_newsencoder(self, embedding_layer):
        ...
        # 2 cái cuối mọt cái cho cate một casii cho sub cate 
    ...
    def _build_naml(self):
        ...
        # his -> qua userencode(newencode(his)) -> ra user present
        user_present = self.userencoder(his_title_body_verts)
        # pred -> qua newencode(pred)  -> ra news present
        # TimeDistributed to apply the same Conv2D layer to each 
        news_present = layers.TimeDistributed(self.newsencoder)(pred_title_body_verts)
        # 2 cái trên gộp vào nhân rồi qua softmax -> ra nhãn 
        ...
        # cái model với score này dịnh nghĩa ở _build_graph -> xử lí train ở base layercó self.model, self.score 
        # cái score này cho evaluate 
        model = keras.Model(
            [
                his_input_title,
                his_input_body,
